...
output "read_replica_public_ip" {
  description = "Public IP address of the read replica (when public access is enabled)"
  value       = local.provision_read_replica && var.public_read_replica ? google_sql_database_instance.replica[0].first_ip_address : null
}
...
output "creds" {
  value = {
    for db in local.databases : db => {
      ...
      read_only_user           = var.public_read_replica ? module.database[db].read_only_user : null
      read_only_user_password  = var.public_read_replica ? module.database[db].read_only_password : null
      ...
      galoy_agents_ro_user     = var.create_galoy_agents_ro_user ? module.database[db].galoy_agents_ro_user : null
      galoy_agents_ro_password = var.create_galoy_agents_ro_user ? module.database[db].galoy_agents_ro_password : null
    }
  }
  sensitive = true
}
