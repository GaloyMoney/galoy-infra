...
resource "aws_iam_policy" "bastion_eks_access" {
  name        = "${local.prefix}-bastion-eks-access"
...
resource "aws_iam_group" "platform_admins" {
  count = length(local.platform_admins) > 0 ? 1 : 0
...
resource "aws_iam_policy" "platform_admin" {
  ...
  description = "Platform admin permissions"
...
resource "aws_iam_group_policy_attachment" "platform_admin" {
  count      = length(local.platform_admins) > 0 ? 1 : 0
  group      = aws_iam_group.platform_admins[0].name
